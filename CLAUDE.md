...
- `AppState.kt` — синглтон на процесс (`AppState.get`), состояние для Compose, фоновые задачи, история
  (с показаниями датчиков для тренда), поездки (`TripLive`/`Trip`, расход по PID 5E, иначе по MAF), опрос датчиков
  (`startPolling`), измерения напряжения (`recordVolt`, раз в минуту + просадки), голосовые предупреждения
  (`checkAlarms`, TextToSpeech ru, только при записи поездки), стирание кодов сохраняет `ClearEvent`.
...
- `ui/Extra.kt` — карточки 0.5 и экраны «Данные с машины» (`DetailsScreen`: мониторы, счётчики, самотесты, блоки с VIN,
  все датчики, адаптер) и «Приборная панель» (`DashScreen`).
...
- Версия 0.5 (18.09.2026) на живой машине ещё не проверена: режим 06, маски PID, 22 F190 по блокам, тест адаптера,
  фото приборки на qwen. Смотреть лог консоли после первой проверки.
