...
- `Blackbox.kt` + `ui/Journal.kt` (1.1) — чёрный ящик: в `AppState.bbBuffer` кольцевой буфер последней минуты по 11 параметрам
  (`BbSample.KEYS`), `recordEvent(kind, title, detail)` отмечает событие (новая ошибка в пути, перегрев, пропажа зарядки,
  перезаряд), через 30 с хвост дописывается и запись уходит в `Prefs.saveBlackbox` (8 последних). `changes()` считает,
  что изменилось за 10 секунд до события, это же уходит нейронке. Экран: список, графики `Sparkline` с чертой в момент события.
- `Service.kt` (1.1) — проверка честности сервиса: `ServiceCatalog.works` (16 работ), `ServiceMetrics.from(snap, warmups)`
  снимает выжимку (коды, коррекции, холостой, дроссель, второй зонд, напряжения, максимум прогрева, счётчики пропусков
  по цилиндрам из режима 06). `addVisit` пишет «до», следующая проверка через `closeVisit` пишет «после», `ServiceAudit.audit`
  выносит вердикт по каждой работе (подтверждается / не подтверждается / проверить нечем). Итог уходит нейронке.
- Фото машины (1.1): `CarImage.bestImage` берёт все картинки статьи Википедии и выбирает по имени файла, отсекая такси,
  полицию, салон, схемы и логотипы; заглавное фото используется только как запасной вариант. Грузится по VIN сразу,
  ещё до разбора нейронкой.

- `Garage.kt` + `ui/GarageScreen.kt` (1.2) — гараж на несколько машин: машина = VIN, у каждой свои журналы. В `Prefs`
  ключи из `Garage.CAR_KEYS` пишутся с префиксом `<carId>/`, `carId` меняет `AppState.switchCar` (перечитывает историю,
  поездки, напряжения, прогревы, пуски, баки, визиты, чёрный ящик). `noticeCar` при чтении VIN сам переключает машину.
  Облачный гараж: сервер хранит `prefs.exportCar()` по коду гаража (`Prefs.garageCode`, 16 символов), эндпоинты
  `/api/garage/put|get|list` в `server/forum/server.py` (512 КБ на машину, 12 машин на код). Автовыгрузка после проверки
  при `Prefs.garageAuto`. Перенос на другой телефон: ввести тот же код и нажать «Что в облаке» → «Забрать».
- Охрана (1.2): `Prefs.guard`, `AppState.guardOnStart` в `engineStarted`. Пока телефон в радиусе адаптера, пуск двигателя
  даёт уведомление, голос и запись в чёрный ящик. Честное ограничение: если телефон далеко, узнать о пуске неоткуда.
